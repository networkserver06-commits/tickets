...
function createSession(username: string, secret: string) {
  const payload = Buffer.from(
    username + "|" + (Date.now() + SESSION_TTL_MS),
    "utf8"
  ).toString("base64url");
...
export function getAdminUsername(req: {
  ...
  if (
    !payload ||
    !signature ||
    !safeEqual(signature, sign(payload, config.sessionSecret))
  )
    return null;
  try {
...
export function authenticateAdmin(username: string, password: string) {
  ...
  if (
    ...
    return { configured: true as const, token: null };
  return {
    configured: true as const,
    token: createSession(config.username, config.sessionSecret),
    username: config.username,
  };
}
...
export function setAdminCookie(
  ...
  const isSecure = Array.isArray(forwardedProto)
