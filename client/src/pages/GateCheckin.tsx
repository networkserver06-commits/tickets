...
export default function GateCheckin() {
  ...
  async function checkin(requestedTicketId = ticketId, requestedPhone = phone) {
    ...
    try {
      const response = await fetch("/api/gate/checkin", { method: "POST", headers: { "Content-Type": "application/json", "x-gate-pin": pin }, body: JSON.stringify({ ticketId: nextTicketId, phone: nextPhone }) });
      const body = await response.json().catch(() => ({}));
      if (response.ok) { sessionStorage.setItem("passage_gate_pin", pin.trim()); setResult("approved"); setTicket(body.ticket || null); setMessage("Entry approved"); }
