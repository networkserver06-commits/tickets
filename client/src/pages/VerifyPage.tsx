...
export default function VerifyPage() {
  ...
  const id = params?.id || "";
  const { loading: authLoading, isAuthenticated, login } = useAdminAuth();
  const [state, setState] = useState<"loading" | "valid" | "used" | "missing" | "error">("loading");
  useEffect(() => { if (authLoading || !isAuthenticated) return; let active = true; fetch("/api/tickets/verify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ticketId: id }) }).then(response => { if (!active) return; if (response.ok) setState("valid"); else if (response.status === 409) setState("used"); else if (response.status === 404) setState("missing"); else setState("error"); }).catch(() => active && setState("error")); return () => { active = false; }; }, [id, authLoading, isAuthenticated]);
  if (authLoading) return <AdminLoginScreen loading onLogin={login} />;
  if (!isAuthenticated) return <AdminLoginScreen onLogin={login} heading="Verify guests with confidence." description="Sign in to securely scan and admit ticket holders." />;
