...
import { Activity, CheckCircle2, CircleDollarSign, ClipboardList, Download, RefreshCw, Settings, Ticket, TrendingUp, XCircle } from "lucide-react";
...
export default function Home() {
  ...
  return <DashboardLayout>{location === "/" ? <Overview summary={summary} loading={loading} error={error} onRetry={() => window.location.reload()} /> : <SubPage path={location} summary={summary} loading={loading} error={error} />}</DashboardLayout>;
}
...
function OrdersTable({ orders, tickets, loading }: { orders: Order[]; tickets: TicketRow[]; loading: boolean }) { return <Card className="border-0 bg-white shadow-[0_16px_45px_rgba(15,23,42,0.06)]"><CardHeader className="flex flex-row items-center justify-between"><div><CardTitle className="text-base">Recent orders</CardTitle><p className="mt-1 text-sm text-slate-500">The latest payment and admission activity</p></div><Button variant="ghost" className="gap-2 text-indigo-600 hover:bg-indigo-50"><Download className="h-4 w-4" /> Export</Button></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full min-w-[620px] text-left text-sm"><thead className="border-y border-slate-100 bg-slate-50/70 text-xs uppercase tracking-wider text-slate-400"><tr><th className="px-6 py-3 font-semibold">Order</th><th className="px-6 py-3 font-semibold">Buyer</th><th className="px-6 py-3 font-semibold">Amount</th><th className="px-6 py-3 font-semibold">Status</th><th className="px-6 py-3 font-semibold">Date</th></tr></thead><tbody className="divide-y divide-slate-100">{loading ? [1,2,3].map(i => <tr key={i}><td colSpan={5} className="px-6 py-5"><div className="h-4 animate-pulse rounded bg-slate-100" /></td></tr>) : orders.length ? orders.map(order => { const related = tickets.filter(ticket => ticket.orderId === order.id); const status = related.some(ticket => ticket.status === "valid") ? "valid" : "used"; return <tr key={order.id} className="transition-colors hover:bg-slate-50/70"><td className="px-6 py-4 font-semibold text-slate-800">#{order.id.slice(-8)}</td><td className="px-6 py-4 text-slate-500">{order.buyerEmail}</td><td className="px-6 py-4 font-medium text-slate-800">{money(order.totalAmount)}</td><td className="px-6 py-4"><Badge className={status === "valid" ? "border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-50" : "border-slate-200 bg-slate-100 text-slate-500 hover:bg-slate-100"}>{status === "valid" ? "Valid" : "Used"}</Badge></td><td className="px-6 py-4 text-slate-400">{dateLabel(order.createdAt)}</td></tr>; }) : <tr><td colSpan={5}><EmptyState icon={ClipboardList} title="No orders yet" description="Successful Paystack payments will appear in this table." /></td></tr>}</tbody></table></div></CardContent></Card>; }
